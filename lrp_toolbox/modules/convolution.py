...
class Convolution(Module):
    ...
    def forward(self,X,padding=False,lrp_aware=False):
        '''
        ...
        #apply padding - Team DeepFault
        #TODO: Should be automatically handled
        #padding=True
        if padding:
            X = np.pad(X, ((0,0), (2,2), (2,2), (0,0)), 'constant', constant_values=(0.,)) # FOR LENET MODELS
        ...
        #for ttt in range(X.shape[1]):
        #    print(X[0,ttt,:,0])
        #print(X[:, 4*hstride:4*hstride+hf: , 4*wstride:4*wstride+wf: , : ])
